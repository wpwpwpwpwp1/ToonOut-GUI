...
Use `%LOCALAPPDATA%\ToonOut\models` by default. Show actual on-disk installation state in the main header. Before the first download, show the location and allow the user to choose a writable folder. Persist the choice with `QSettings` and pass it explicitly as the Hugging Face Hub and remote-module cache location.
